...
from transformers import SchedulerType, get_scheduler, Adafactor
import copy
try:
...
def parse_args():
    ...
    parser.add_argument(
        "--adam_8bit", action="store_true", default=False,
        help="Use 8-bit Adam optimizer for memory-efficient training (requires bitsandbytes library)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dataset_dir", type=str, default="~/.cache/huggingface/datasets",
    ...
    parser.add_argument(
        "--optimizer", type=str, default="adamw", choices=["adamw", "adafactor", "adam8bit"],
        help="Optimizer to use: 'adamw' for AdamW, 'adafactor' for Adafactor, or 'adam8bit' for 8-bit Adam."
    )
...
def main():
    ...
    prompt_list = dataset['train']['prompt']
    ...
    raw_dataset_train = HuggingfaceDataset.from_pandas(pd.DataFrame({
        'prompt': prompt_list,
    ...
    model = DPOAudioDiffusion(
        args.text_encoder_name, args.scheduler_name, args.unet_model_name, args.unet_model_config,
        args.snr_gamma, args.freeze_text_encoder, args.uncondition, args.beta_dpo
    )
    ...
    with accelerator.main_process_first():
        ...
        eval_dataset = DPOText2AudioDataset(dataset["validation"], prefix, 'prompt', 'chosen', 'rejected', args.num_examples)
    ...
    accelerator.print("Num instances in train: {}, validation: {}, test: {}".format(
        train_dataset.get_num_instances(), eval_dataset.get_num_instances(), test_dataset.get_num_instances()
    ))
    ...
    if args.optimizer == "adafactor":
        ...
        accelerator.print("Using Adafactor optimizer for memory-efficient training.")
    elif args.optimizer == "adam8bit":
        if bnb is None:
            raise ValueError("bitsandbytes library is required for 8-bit Adam optimizer but is not installed.")
        ...
        accelerator.print("Using 8-bit Adam optimizer for memory-efficient training.")
    else:
    ...
    logger.info(f" FP16 mixed precision = {args.fp16}")
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process and args.checkpointing_steps == "best":
            ...
            if (epoch + 1) % args.save_every == 0 and sft_epochs < 0:
                accelerator.save_state("{}/{}".format(args.output_dir, "epoch_" + str(epoch + 1)))

        if accelerator.is_main_process and args.checkpointing_steps == "epoch" and sft_epochs < 0:
            accelerator.save_state("{}/{}".format(args.output_dir, "epoch_" + str(epoch + 1)))
